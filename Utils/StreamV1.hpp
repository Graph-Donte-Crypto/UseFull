...
namespace utils {
	...
	template<typename TypeFrom, typename TypeTo>
	void cmpstatic_lazy(TypeFrom *, TypeTo *) {
		printf("CALL ZERO cmpstatic!!!\n");
		exit(1);
	}

	template<typename TypeFrom>
	TypeFrom * ref_to_type(TypeFrom &) {return (TypeFrom *)0;}

	template<typename TypeFrom>
	void cmpstatic_lazy(TypeFrom *, TypeFrom *){}
	...
	struct Stream {
		...
		TypeTo & getNext() {
			//TODO: ALARM! 
			//object = source.getNext();

			//cmpstatic_lazy((decltype(source.getNext()))0, (TypeTo &)0);
			//cmpstatic_lazy(ref_to_type<decltype(source.getNext())>(0), (TypeTo *)0);
		...
		Stream(const SourceType & _source, const Lambda & _lambda, TypeFrom *, TypeTo *) :
			source(_source),
			lambda(_lambda),
			object()
		{
	...
	struct StreamWhile : public parent(StreamWhile) {
		...
		StreamWhile(SourceType & _source, const Lambda & _lambda, TypeFrom *, TypeTo *)
	...
	struct StreamSince : public parent(StreamSince) {
		...
		StreamSince(SourceType & _source, const Lambda & _lambda, TypeFrom *, TypeTo *)
	...
	struct StreamAccept : public parent(StreamAccept) {
		...
		StreamAccept(SourceType & _source, const Lambda & _lambda, TypeFrom *, TypeTo *, size_t num)
	...
	auto GetStream(CoCandidateEnumeration<Type> auto & coll) {
		return StreamIterator(coll, &empty_lambda, (Type *)0, (Type *)0);
	}
}
#endif
