...
namespace str {
	namespace Replace {
		...
		char * First(char * where, const char * what, const char * with_what) {
		...
		char * Repeat(char * where, const char * what, const char * with_what) {
			char * pos = where;
			while (( pos = strstr(pos, what) ) != nullptr) {
				Replace::Standart(pos, what, with_what);
			}
			return where;
		}
		//TODO: IDK what is that
		char * Raw(char * where, int pos, const char * with_what, int shift) {
			if (shift < 0) {
				int pk = pos - shift;
